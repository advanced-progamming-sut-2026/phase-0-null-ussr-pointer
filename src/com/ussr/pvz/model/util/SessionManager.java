...
public final class SessionManager {
    ...
    private static final Path SESSION_FILE =
            Path.of("src/resources/current_session.txt");

    private static String token;

    private SessionManager() {}

    public static void saveSession(String sessionToken, boolean persist) {
        token = sessionToken;
        ...
        if (!persist) {
            try {
                ...
                System.err.println("Failed to clear stale session file: " + e.getMessage());
            }
            return;
        }

        try {
            Files.writeString(SESSION_FILE, sessionToken);
        } catch (IOException e) {
    ...
    public static String getToken() {
        ...
        try {
            String savedToken = Files.readString(SESSION_FILE).trim();
            if (savedToken.isBlank()) return null;
            token = savedToken;
            return token;
        } catch (IOException e) {
