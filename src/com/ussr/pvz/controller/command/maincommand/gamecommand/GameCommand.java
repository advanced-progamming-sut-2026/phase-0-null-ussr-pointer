...
public enum GameCommand {
    ...
    CHEAT_SPAWN_ZOMBIE("^cheat\\s+spawn-zombie\\s+-t\\s+(?<type>.+?)\\s+-l\\s*\\(\\s*(?<x>\\d+)\\s*,\\s*(?<y>\\d+)\\s*\\)(?:\\s+(?<glowing>-g))?(?:\\s+.*)?$"),
    CHEAT_DECREASE_HEALTH("^cheat\\s+decrease-health\\s+-t\\s+(?<type>.+?)\\s+-a\\s+(?<amount>\\d+)(?:\\s+.*)?$"),
    ...
    // === Minigame Commands ===
    SWAP_PLANTS("^swap\\s+\\(\\s*(?<r1>\\d+)\\s*,\\s*(?<c1>\\d+)\\s*\\)\\s+with\\s+\\(\\s*(?<r2>\\d+)\\s*,\\s*(?<c2>\\d+)\\s*\\)(?:\\s+.*)?$"),
    ...
    PLACE_ZOMBIE("^place\\s+zombie\\s+-t\\s+(?<type>.+?)\\s+-l\\s*\\(\\s*(?<x>\\d+)\\s*,\\s*(?<y>\\d+)\\s*\\)(?:\\s+.*)?$"),
    PLANT_FROM_SEEDPACK("^plant\\s+from\\s*\\(\\s*(?<sx>\\d+)\\s*,\\s*(?<sy>\\d+)\\s*\\)\\s+to\\s*\\(\\s*(?<x>\\d+)\\s*,\\s*(?<y>\\d+)\\s*\\)(?:\\s+.*)?$"),
